...
単一のポーズだけでなく、`(N, 4, 4)` のようにNumPyのブロードキャストを利用して一括変換できる機能を持ちます。ほとんどのメソッドがバッチ処理に対応しており、複数のポーズを効率的に処理できます:
...
| **正規化 (normalize)** | O(n²) | グラム・シュミット法による直交化。 |
...
p_euler_with_pos = Pose6DOF.from_euler([90, 0, 0], 'xyz', degrees=True, pos=[1, 2, 3])  # 位置も指定可能
...
# 6. 数値的なドリフトをリセット
# normalizeメソッドはグラム・シュミット法で正規化します
# 計算コスト: O(n²)（グラム・シュミット法による直交化）
